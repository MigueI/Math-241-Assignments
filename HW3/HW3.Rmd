...
Let's find our start and end dates for the academic year (plus one hour for daylight savings).
...
frijan <- lastDayFinder(1, 1, six.years) + ddays(4)  + dweeks(15) + dhours(1)
...
# Make column with week number since beginning (1 to ~300).
jukebox$week <- week(jukebox$date_time) + 
  52*year(jukebox$date_time) - 52*year(jukebox$date_time[1])
...
```{r, echo=FALSE, fig.width=12}
# Make column with month count, starting at 1.
jukebox$month <- month(jukebox$date_time) + 12*year(jukebox$date_time) - 12*year(jukebox$date_time[1]) -11
...
ggplot(juke_month, aes(month, count, fill = artist)) + 
  geom_bar(stat = "identity", position = "dodge") + 
  xlab("Month") + ylab("Song Count") + ggtitle("Songs by Month")
```
...
topblues <- subset(jukebox, sunblues == 1) %>%
  group_by(track) %>%
  summarize(sun_percent = n()/12454) %>% 
  arrange(sun_percent) %>%
  top_n(15, sun_percent)

# Find top songs NOT during sunday blues
topnoblues <- subset(jukebox, sunblues == 0) %>%
  group_by(track) %>%
  summarize(nosun_percent = n()/218110)
...
In red we have Sunday night blues period, and the blue represents the rest of the week (not including the blues window). There is definately a notable change in percentages in most songs. 
